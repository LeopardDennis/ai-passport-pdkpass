...
最多记住 5 个网络，关机不丢失；优先连接上次成功的网络。断线后，每个已保存网络
尝试两次（每次最多 15 秒），全部失败则进入配网。配网期间，没有手机连接临时热点时，
每隔 60 秒启动一轮已保存网络重试。当前连接正常时，不会因为另一个网络信号更强就切换。

添加网络时，将设备带到已保存网络不可用的地方，再用配网页添加即可。同名 Wi-Fi 更新
