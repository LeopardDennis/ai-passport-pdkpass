...
- 上、下、确定与长按操作增加轻声的双音像素风提示音。音效由独立任务播放，
  快速连按只保留最新提示，播放后关闭 codec，不拖慢按键与熄屏。

- 所有内置分站首页名称按可用宽度适配，同时保持可读字高，包括澳大利亚、荷兰和
  沙特阿拉伯站；界面中的斯帕赛道名简写为 `SPA`。底部操作提示
  完整显示，文字在票券内左右等距。

- 赛道详情页顶部标题按票券宽度适配，不再因名称和轮次较长就缩成 8 像素小字；
  47 站内置赛道标题统一字体和可见高度，长名称只按宽度适配；分隔点移到文字
  垂直中线。

- 加大赛道标题的分隔点；成绩页的车手姓名占据号码右侧的完整一行，三字代码移到
  第二行与车队名并列，避免较长姓名被截断。

- 成绩页显示车手名和姓，例如 `MAX VERSTAPPEN`；较长的全名按单行宽度适配。
  积分榜继续显示姓氏，并迁移已有赛季缓存，保留已保存的赛历和积分。

- 成绩卡片保留原有车队色背景；车队名称使用本队颜色的深色或浅色版本，
  包括梅奔蓝，与车手姓名区分，同时兼顾深色和浅色车队背景。

- 成绩页与赛道详情页统一使用等高、按宽度适配的轮次标题，每站的分隔点都与文字
  垂直居中。

- 首页短按上/下切换上一场/下一场；长按上打开车手积分，长按下打开赛历。
  短按确定查看当前显示的分站详情。无操作 90 秒熄屏后，下次唤醒显示当周比赛。

- 用户主动打开历史分站时，分节发现或未完成的前三名下载失败满五分钟后允许重试，
  不再受后台补齐任务的一天退避限制；在成绩或详情页唤醒屏幕也会重新请求该站。
  此改动针对照片中的 R13 FP1 等待页及串口捕获的传输超时，不增加其他历史分站的
  自动重试频率。OpenF1 请求失败时还会记录不含查询参数的接口名称，方便判断超时
  发生在分节、车手资料还是成绩接口。

- 联网但当前分节仍无缓存时，周末结束前显示 `RESULT PENDING`，已结束的分站显示
  `SYNC PENDING`。仅凭 Wi-Fi 已连接不再宣称 OpenF1 正在下载。

- 将 OpenF1 的成绩、车手积分、分节发现及前三名车手资料改为逐条解析，避免先缓存
  整段响应。真机曾在 HTTP 200、已收到约 7.9 KB 时尝试扩容到 16 KB，而最大连续
  空闲内存只有 7.7 KB；这次改动移除了该扩容路径。仍保留响应校验和重试逻辑，
  实际同步成功情况待新固件真机验证。

- 为 HTTP 请求和下载后解析增加仅在失败时输出的诊断：记录失败阶段、状态、错误、
  响应大小、清理客户端前后的空闲内存及最大连续可分配块，还有开机以来的最低值。
  用主机故障注入覆盖成功、截断、超限、格式错误、限流、传输及分配失败；疑似
  HTTPS 内存问题仍待真机测量确认，本次不猜测修复。

- 网络菜单的同步日期只对应当前赛季。离线跨年后，上赛季日期不再冒充新赛季已更新；
  改为显示 `CAL 2027 NO SYNC` 或 `RESULT 2027 NO SYNC`。当前赛季已有缓存但
  日期未知时仍显示 `CACHE DATE?`。

- 区分“升级前已有缓存但没有记录同步日期”和“从未缓存”。网络菜单对旧缓存显示
  `CAL CACHE DATE?` / `RESULT CACHE DATE?`，无缓存显示 `NEVER`，成功记录
  同步时间后显示北京时间日期。

- 减少列表切换及重复网络、电量状态导致的无谓全屏失效。使用分块缓冲区的模拟器中，
  积分榜同页移动一行从刷新 76,800 像素降至约 15,848 像素，网络菜单选项移动从
  49,984 降至约 16,800；未变化的状态和电量不再触发刷新。熄屏时停止读取电量计，
  按键唤醒后恢复，不更改 ADC 按键扫描频率。

- 断电后恢复的时间仅作估计：首页日期加 `~`，本次开机完成校时前不据此自动
  切换分站或年份。用 `WIFI OK` 表示连接状态，并在网络菜单分别保存和显示
  赛历、成绩最近一次成功更新的日期。

- 拉开 27 条赛道的背景配色，尤其是原先接近的红、蓝、青和绿色组合；
  每条赛道的状态栏及赛历选中项也改用独立强调色。相同赛道在 2026、2027
  赛历中仍保持同一套配色。2027 巴林站改为深梅紫色背景，葡萄牙站改为
  更明亮的紫色背景。

- 内置 2027 官方日期级赛历（24 站、10 个冲刺周末），北京时间跨年后离线也能选择，
  保留已接受的同年或更新缓存，复用赛道配色。未知时间、圈数和积分保持待公布。
  具体分节时间获取前按举办地周末结束后的当地零点切站。
  模拟器新增 `--year 2027`，本次不处理 HTTPS 内存问题。

- 新增独立于年份的 27 条赛道目录，统一稳定 ID、别名、轮廓、长度及配色，
  补齐波尔蒂芒、伊斯坦布尔、萨基尔及吉达离线轮廓。下载赛历及缓存不再依赖
  上一赛季取得赛道资料；圈数仍属于赛事，未知圈数不隐藏已知长度。

- 修复配网时重复离线状态触发成绩任务与网络任务相互唤醒，
  导致 CPU 忙循环和看门狗报警的问题；仅在连接状态变化时唤醒成绩任务。

- 增加随亮灭屏切换的 80–160 MHz 电源管理和自动轻睡眠，亮屏、连接/配网及
  USB 控制台连接时保持唤醒。LVGL 改用单调时钟，熄屏暂停绘图但保留北京时间
  切站计时。ADC 按键仍每 20 毫秒扫描，不使用深睡眠。
- 两个数据任务近期工作完成后主动关闭空闲 Wi-Fi，并与 HTTP 事务互斥。
  任务到期或打开未缓存页面时重新联网；失败和手动取消后仍保持离线。
  首次 NTP 校时最多等待 60 秒，保留主动配网及三分钟/十分钟热点超时。

- 全界面小字宽度增加 25%、高度约增加 50%，覆盖配网、状态栏、赛程和成绩；
  赛历每页五行且每站保持单行、积分榜每页五行，精简操作提示，为车手姓名
  留出更宽空间。增加文字内边距，长赛历条目横向适配，不换行。

- 配网 WPA2 密码缩短为每次随机生成的八位大写字母和数字，避开易混淆字符；
  生成前先启动无线电以提供硬件随机熵源。

- 主动配网热点改为 `192.168.9.1/24`，同步修改网关、DHCP 子网和设备浏览器提示。

- 首页长按 OK 打开网络菜单，分别提供已保存网络重试和主动配网。已保存网络扫描
  一轮后失败即关闭 Wi-Fi。热点无人连接三分钟或累计十分钟自动关闭，支持取消；
  获取 IP 并保存成功即关闭配网。保留屏幕自动熄灭，显示不因操作重置的配网倒计时。

- 修复首次配网在连接前卡住：未连接时跳过断开等待，取消进行中的连接时通过
  站点停止事件隔离旧事件。保留五组网络记忆，仍在获取 IP 后才保存新凭据。
  配网页面显示认证失败、找不到网络、安全模式、连接/IP 超时和保存失败提示。
  切换进行中的连接可能短暂中断配网热点，重新连接热点即可重试。

- 补齐内置 2026 赛季全部 23 站，加入 R1–R12 的日期、北京时间与赛道信息，
  模拟器共用同一份数据。修复旧版 R13–R23 缓存，同时保留更新后的赛程与积分；
  其他年份及联网下载的不同赛历结构保持不变。列表标题改为 `SEASON CALENDAR`，
  明确包含历史比赛。历史前三名仍需联网同步后才能离线查看。

- 修复启动界面卡死：为 LVGL 对象和文字缩放临时图层预留 64 KB，
  原生模拟器使用相同预算，并新增界面内存回归测试。

- 低电量图标使用对比色轮廓和浅色内部，确保红色状态栏上仍然清晰；
  网络任务等待事件或有效截止时间，避免空闲时每秒轮询。

- 电量文字改为圆角电池图标，内部显示电量填充与百分比数字；未知电量显示空轮廓，
  0–20% 时浅色内部显示红色数字和剩余填充，21–100% 使用正常主题色。
  模拟器默认使用模拟值 88，支持 `--battery -1..100`。
- 网络文字、日期和电池统一在状态栏背景内居中，不计算边框和阴影；新增模拟器
  像素回归检查。
- 最多保存 5 个 Wi-Fi，断电保留，优先连接上次成功的网络，断线后依次尝试其他网络。
  兼容旧版单网络凭据，配网热点没有手机连接时后台重试已保存网络。
- 新增网络配置校验、更新、淘汰和重试顺序的主机测试。
- 修复正赛结束后 30 分钟的积分刷新时点，并为同步失败增加有界重试；
  部分数据下载失败时保留已确认赛程。
- 当前周末成绩优先更新，历史数据采用独立重试截止时间；成绩缓存从 v2
  迁移到 v3 时保留前三名，并串行处理赛季与成绩事务，避免赛季变化时写入旧结果。
- 赛季 JSON 改为逐对象解析，小响应缓冲按需增长且限制上限，两个数据服务
  共用 HTTP 限流退避。
- 配网使用随机热点密码、拒绝重叠提交并仅保存实际验证的凭据；本次开机
  已同步的时钟在 Wi-Fi 重连时继续可用，并增加 NTP 错误状态。
- 复用赛历和积分榜可见行，电池采样移出 LVGL，按键通过队列派发，
  熄屏后停止空闲定时器轮询；USB 截图改为可选调试功能并限制输出超时。
